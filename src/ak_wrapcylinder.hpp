 ...
 */
class OSIMSIMULATION_API ak_WrapCylinder : public WrapObject {
OpenSim_DECLARE_CONCRETE_OBJECT(ak_WrapCylinder, WrapObject);
public:
...
public:
    ak_WrapCylinder();
    virtual ~ak_WrapCylinder() override = default;
...
protected:
    int wrapLine(const SimTK::State& s,
                 SimTK::Vec3& aPoint1,
                 SimTK::Vec3& aPoint2,
    ...
    void extendAddToSystem(SimTK::MultibodySystem& system) const override {
        ...
        /*
        SimTK::CableTrackerSubsystem& cables = _model->updCableSubsystem();
        const auto* s = dynamic_cast<const WrapObjectSet*>(&this->getOwner());
        if (!s) {
            OPENSIM_THROW(Exception, "not wrap object set");
        }
        const auto* pof = dynamic_cast<const PhysicalOffsetFrame*>(&s->getOwner());
        if (!pof) {
            OPENSIM_THROW(Exception, "not a pof");
        }
        const PhysicalOffsetFrame& parent = *pof;
        auto path = new SimTK::CablePath{
                cables,
                parent.getMobilizedBody(),
                SimTK::Vec3(0,0,0),   // origin
                parent.getMobilizedBody(),
                SimTK::Vec3(1,1,1),  // termination
        };
        */
    }
...
private:
    ...
    void _make_spiral_path(SimTK::Vec3& aPoint1,
                           SimTK::Vec3& aPoint2,
                           bool far_side_wrap,
...
* Default constructor.
*/
ak_WrapCylinder::ak_WrapCylinder() : WrapObject()
{
    constructProperties();
}
...
*/
void ak_WrapCylinder::constructProperties()
{
    constructProperty_radius(1.0);
    constructProperty_length(1.0);
}

void ak_WrapCylinder::extendScale(const SimTK::State& s, const ScaleSet& scaleSet)
{
...
*/
void ak_WrapCylinder::extendFinalizeFromProperties()
{
    // Base class
 ...
 */
const char* ak_WrapCylinder::getWrapTypeName() const
{
    return wrapTypeName;
}
 ...
 */
string ak_WrapCylinder::getDimensionsString() const
{
 ...
 */
int ak_WrapCylinder::wrapLine(const SimTK::State&,
                              SimTK::Vec3& aPoint1,
                              SimTK::Vec3& aPoint2,
                              const PathWrap& aPathWrap,
                              WrapResult& aWrapResult,
 ...
 * @param aWrapResult The result of the wrapping (tangent points, etc.)
 */
void ak_WrapCylinder::_make_spiral_path(SimTK::Vec3& aPoint1,
                                                 SimTK::Vec3& aPoint2,
                                                 bool far_side_wrap,
                                                 WrapResult& aWrapResult) const
{
 ...
 */
void ak_WrapCylinder::_calc_spiral_wrap_point(const SimTK::Vec3& r1a,
                                                         const SimTK::Vec3& axial_vec,
                                                         double m[4][4],
                                                         const SimTK::Vec3& axis,
                                                         double sense,
 ...
 */
bool ak_WrapCylinder::_adjust_tangent_point(SimTK::Vec3& pt1,
                                                      SimTK::Vec3& dn,
                                                      SimTK::Vec3& r1,
                                                      SimTK::Vec3& w1) const
{
...
// in ModelVisualizer
void ak_WrapCylinder::generateDecorations(bool fixed, const ModelDisplayHints& hints, const SimTK::State& state,
